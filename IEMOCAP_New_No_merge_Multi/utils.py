...
def Feature(args,data):
    ...
    for i in range(len(data)):
        #print(data[i])
        input_label_self_Emotion_Change.append(data[i]['Self_Emotion_Change'])

    input_label_Inter_Emotion_Change = []
    for i in range(len(data)):
        input_label_Inter_Emotion_Change.append(data[i]['Inter_Emotion_Change'])
...
def Get_data(data,train,test,args):
    ...
    label_2 = np.array(input_train_label_2).reshape(-1, 1)
    label_test_2 = np.array(input_test_label_2).reshape(-1,1)

...
parser.add_argument('--out_class_2', type=int, default=2)
...
def Train(epoch,criterion_two_class,criterion_two_class_1):
    train_loss = 0
    dia_net_a.train()
    dia_net_b.train()
    dia_net_all.train()
    ...
    for batch_idx, (data_1, data_2,data_3,data_1_1,data_2_1,data_3_1, target, target_1,target_2) in enumerate(train_loader):
        if args.cuda:
            data_1, data_2, data_3, data_1_1, data_2_1, data_3_1, target, target_1,target_2 = data_1.cuda(), data_2.cuda(), data_3.cuda(), data_1_1.cuda(), data_2_1.cuda(), data_3_1.cuda(), target.cuda(),target_1.cuda(),target_2.cuda()
        # data (batch_size, step, 88)
        # target (batch_size, 1)
        data_1, data_2, data_3, data_1_1, data_2_1, data_3_1, target, target_1, target_2 = Variable(data_1), Variable(data_2),Variable(data_3),Variable(data_1_1),Variable(data_2_1),Variable(data_3_1),Variable(target),Variable(target_1),Variable(target_2)

        target = target.squeeze()
        target_1 = target_1.squeeze()
        target_2 = target_2.squeeze()
        ...
        #line_out_2 = output_net_2(line_input)
        ...
        #loss_3 = criterion_two_class_1(line_out_2, target_2.long())
...
def Test():
    ...
    with torch.no_grad():
        for batch_idx, (data_1, data_2, data_3, data_1_1, data_2_1, data_3_1, target, target_1, target_2) in enumerate(
            ...
            # data (batch_size, step, 88)
            # target (batch_size, 1)
            data_1, data_2, data_3, data_1_1, data_2_1, data_3_1, target, target_1, target_2 = Variable(
...
for index, (train, test) in enumerate(kf.split(data)):
    ...
    train_loader, test_loader, class_num_0, class_num_1, class_num_00, class_num_11, input_test_data_id, input_test_label_org, test_len = Get_data(data, train, test, args)
    ...
    criterion_two_class_1 = torch.nn.CrossEntropyLoss(weight=torch.from_numpy(np.array([class_num_00, class_num_11])).float(),size_average=True)
    criterion_two_class_1.cuda()

    f1 = 0
    recall = 0
    for epoch in range(1, args.epochs + 1):
        Train(epoch,criterion_two_class,criterion_two_class_1)

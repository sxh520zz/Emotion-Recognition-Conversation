...
class subDataset(Dataset.Dataset):
    ...
    def __getitem__(self, item):
        ...
        label_2 = torch.Tensor(self.Label_2[item])

        return data_1,data_2,data_3,data_1_1,data_2_1,data_3_1,label,label_1,label_2
...
def Get_data(data,train,test,args):
    ...
    print(len(test_data))
    input_train_data_trad,input_train_data_tran,input_train_label,input_train_label_self_Emotion_Change,input_train_label_Inter_Emotion_Change,_,_ = Feature(args,train_data)
    input_test_data_trad,input_test_data_tran,input_test_label,input_test_label_self_Emotion_Change,input_test_label_Inter_Emotion_Change,input_test_data_id,input_test_label_org = Feature(args,test_data)

    label = np.array(input_train_label).reshape(-1, 1)
    label_1 = np.array(input_train_label_self_Emotion_Change).reshape(-1, 1)
    label_2 = np.array(input_train_label_Inter_Emotion_Change).reshape(-1, 1)

    label_test = np.array(input_test_label).reshape(-1,1)
    label_test_1 = np.array(input_test_label_self_Emotion_Change).reshape(-1,1)
    label_test_2 = np.array(input_test_label_Inter_Emotion_Change).reshape(-1,1)

    class_num_0 = 0
    class_num_1 = 0
    for i in range(len(input_train_label)):
        if(input_train_label_self_Emotion_Change[i] == 0):
            class_num_0 = class_num_0 + 1
        if(input_train_label_self_Emotion_Change[i] == 1):
            class_num_1 = class_num_1 + 1

    class_num_00 = 0
    class_num_11 = 0
    for i in range(len(input_train_label)):
        if(input_train_label_Inter_Emotion_Change[i] == 0):
            class_num_00 = class_num_00 + 1
        if(input_train_label_Inter_Emotion_Change[i] == 1):
            class_num_11 = class_num_11 + 1


    train_dataset = subDataset(input_train_data_trad,input_train_data_tran,label,label_1,label_2)
    test_dataset = subDataset(input_test_data_trad,input_test_data_tran,label_test,label_test_1,label_test_2)
    ...
    return train_loader, test_loader, 1/class_num_0, 1/class_num_1, 1/class_num_00, 1/class_num_11, input_test_data_id[:org_len], input_test_label[:org_len], org_len
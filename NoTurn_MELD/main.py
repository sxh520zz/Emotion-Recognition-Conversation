...
parser.add_argument('--gru_insize', type=int, default=768)
...
def Train(epoch, criterion_two_class):
    ...
    for batch_idx, (data_1_1, data_1_2, data_1_3, data_2_1, data_2_2, data_2_3, target, target_1) in enumerate(
        ...
        # opensmile
        data_1_1 = data_1_1.squeeze(3)
        data_1_2 = data_1_2.squeeze(3)
        data_1_3 = data_1_3.squeeze(3)
        # Bert
        ...
        dia_out_b, dia_hid_b = dia_net_b(data_2_2)

        dia_out_all, _ = dia_net_all(data_2_3)
...
train_loader, test_loader, class_num_0, class_num_1, input_test_data_id, input_test_label_org, test_len = Get_data(
    data, args)
...
criterion_two_class = torch.nn.CrossEntropyLoss(
    weight=torch.from_numpy(np.array([class_num_0, class_num_1])).float(), size_average=True)
